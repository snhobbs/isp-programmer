...
def WriteFlashSector(isp: ISPConnection, chip: ChipDescription, sector: int, data: bytes):
    ...
    for i in range(retries):
        ...
        sleep(ram_write_sleep)
        ram_crc = retry(isp.ReadCRC, count=1, raise_on_fail=False, exception=UserWarning)(ram_address, num_bytes=len(data))
        if data_crc == ram_crc:
    ...
    ram_equal = retry(isp.MemoryLocationsEqual, count=5, exception=(UserWarning, timeout_decorator.TimeoutError))(flash_address, ram_address, chip.sector_bytes)
    if ram_equal:
...
def WriteBinaryToFlash(isp: ISPConnection, chip: ChipDescription, image: bytes, start_sector: int) -> int:
    ...
    assert start_sector + sector_count <= chip.SectorCount
    isp.Unlock()
...
def WriteImage(isp: ISPConnection, chip: ChipDescription, image_file: str):
    '''
    ...
    with open(image_file, 'rb') as f:
        ...
        image = MakeBootable(chip.kCheckSumLocation, prog)
...
def InitConnection(isp: ISPConnection): #, chip):
    ...
    try:
        ...
        boot_code_version = isp.ReadBootCodeVersion()
